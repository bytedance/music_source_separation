...
class Separator:
    ...
    def separate(self, audio):
        ...
        sep_segments = self._forward_in_mini_batches(
            self.model, segments, self.batch_size
        )['wav']
        # (segments_num, channels_num, segment_samples)
    ...
    def _forward_in_mini_batches(self, model, x, batch_size):
        r"""Forward data to model in mini-batch.

        Args:
            model: nn.Module
            x: ndarray, (segments_num, segment_samples)
            batch_size: int

        Returns:
            output_dict: dict, e.g. {
                'wav': (segments_num, channels_num, segment_samples),
                'sp': (segments_num, channels_num, time_steps, freq_bins)}
        """
        output_dict = {}
        ...
        while True:
            ...
            if 'cuda' in self.device:
                batch_waveform = batch_waveform.to(self.device)

            pointer += batch_size

            input_dict = {'waveform': batch_waveform}

            with torch.no_grad():
                model.eval()
                batch_output_dict = model(input_dict)
