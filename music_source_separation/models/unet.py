...
from music_source_separation.models.pytorch_modules import Base, init_bn, init_layer, act, Subband
...
class UNet(nn.Module, Base):
    def __init__(self, input_channels, target_sources_num):
        ...
        self.after_conv2 = nn.Conv2d(in_channels=32, out_channels=input_channels * self.subbands_num * target_sources_num * self.K, 
    ...
    # def subband_analysis(self, x):
    #     (_N, _C, _T, _F) = x.shape
    #     x = x.reshape(_N, _C, _T, self.subband, _F // self.subband)
    #     x = x.transpose(2, 3)
    #     x = x.reshape(_N, _C * self.subband, _T, _F // self.subband)
    #     return x

    # def subband_synthesis(self, x):
    #     (_N, _C, _T, _F) = x.shape
    #     x = x.reshape(_N, _C // self.subband, self.subband, _T, _F)
    #     x = x.transpose(2, 3)
    #     x = x.reshape(_N, _C // self.subband, _T, _F * self.subband)
    #     return x
    ...
    def feature_maps_to_wav(self, x, sp, sin_in, cos_in, audio_length):
        ...
        x = x.reshape(batch_size, self.target_sources_num, self.input_channels, self.K, time_steps, freq_bins)
        ...
        shape = (batch_size * self.target_sources_num * self.input_channels, 1, time_steps, freq_bins)
        ...
        # Reshape.
        wav_out = wav_out.reshape(batch_size, self.target_sources_num * self.input_channels, audio_length)
        # (batch_size, target_sources_num * input_channels, segments_num)
